...
![Mail 桌面工作台](docs/images/dashboard.png)
...
| 🚀 | OAuth2 邮件传输 | IMAP XOAUTH2 收件、SMTP OAuth2 发件以及 Graph `Mail.Send` 回退。 |
...
```mermaid
flowchart LR
  Browser[React Web 应用] -->|签名 HttpOnly Cookie| API[Express API]
  API --> Identity{数据归属范围}
  Identity --> User[登录用户]
  Identity --> Guest[独立游客]
  API --> DB[(SQLite)]
...
- 原生邮件 HTML 会先清理，再放入沙箱 iframe 中显示。
...
回归测试覆盖账号导入导出、浏览器路径、首次部署、五分钟邮件验证码、公告推送、安全 HTML 渲染以及多用户账号隔离。
...
Mail/
├── src/                 React 应用与响应式界面
├── src/components/      发信与通用界面组件
├── server/              Express、身份、SQLite、OAuth2、IMAP、SMTP
├── public/              品牌资源与本地字体
...
- [CN-Root/OutlookPanel](https://github.com/CN-Root/OutlookPanel) — Outlook 账号导入与邮箱工作流参考。
- [oiov/wr.do](https://github.com/oiov/wr.do) — 视觉语言与导航设计参考。
- [Lucide](https://lucide.dev/) — 界面图标。

Mail 是独立项目，与 Microsoft 没有关联，也未获得 Microsoft 官方认可。
